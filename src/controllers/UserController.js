...
const {
  ...
  UserValid,
  WorkerValid,
} = require("../utils/validator/UserValid");
const {
  ForgotPasswordValid,
  ChangePasswordValid,
} = require("../utils/validator/AuthValid");
...
const loginWithGG = async (req, res) => {
  ...
  try {
    ...
    if (emailExists) {
      ...
      if (existingAccount && existingAccount.provider !== "google.com") {
        return failResponse(
          res,
          500,
          `Email này đã được đăng ký với ${existingAccount.provider}. Vui lòng đăng nhập bằng ${existingAccount.provider}`
        );
      }
    }
    ...
    if (accountDoc.exists) {
      ...
      const newAccount = new AccountModel(uid, email, role, "google.com");
    ...
    if (userDoc.exists) {
      ...
      const rawUser = {
        ...
        avatar: picture
          ? picture
          : "https://res.cloudinary.com/dvofgx21o/image/upload/v1754337546/jobs/byhangkho4twacw1owri.png",
        username: name ? name : email.split("@")[0],
      };

      let validated;
      if (role === "user") {
        validated = await UserValid.validateAsync(rawUser, {
          stripUnknown: true,
        });
        await UserService.createUser(validated);
      } else {
        validated = await WorkerValid.validateAsync(rawUser, {
          stripUnknown: true,
        });
...
const createUser = async (req, res) => {
  try {
    ...
    if (role == "user") {
      const validated = await UserValid.validateAsync(rawUser, {
        stripUnknown: true,
      });
      await UserService.createUser(validated);
    } else {
      const validated = await WorkerValid.validateAsync(rawUser, {
        stripUnknown: true,
      });
      await WorkerService.createWorker(validated);
    }
...
const forgotPassword = async (req, res) => {
  try {
    ...
    const validated = await ForgotPasswordValid.validateAsync(rawData, {
      stripUnknown: true,
    });
...
const changePassword = async (req, res) => {
  try {
    ...
    const rawData = req.body;
    const validated = await ChangePasswordValid.validateAsync(
      { email: emailToken, ...rawData },
      { stripUnknown: true }
    );
...
const updateUser = async (req, res) => {
  try {
    ...
    if (req.body.role === "user") {
      const validated = await UserValid.validateAsync(rawData, {
        stripUnknown: true,
      });
      userData = await UserService.updateUser(validated);
    } else {
      const validated = await WorkerValid.validateAsync(rawData, {
        stripUnknown: true,
      });
...
const deleteUser = async (req, res) => {
  try {
    ...
    if (req.body.role === "user") {
      const validated = await UserInfoValid.validateAsync(rawData, {
        stripUnknown: true,
      });
      await UserService.deleteUser(validated.uid);
    } else {
      const validated = await WorkerInfoValid.validateAsync(rawData, {
        stripUnknown: true,
      });
